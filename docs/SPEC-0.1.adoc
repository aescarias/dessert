...
- Describing a binary data format such as a PNG file, a complete FAT filesystem, or formats delivered over-the-wire such as TCP.

Dessert should provide the tools necessary to describe anything from basic sequential data to complex data that includes compression or bit-level access requirements without the boilerplate that is usually expected for these cases. Dessert should provide a standard library of common functions for math, data compression, encoding, and checksums.

Dessert strives to combine tools commonly found in programming languages like C such as control flow, repetition, functions, and standard types, plus elements found in regular expressions such as character groups and greedy matches.
...
Mappings are ordered collections of one or more key-value pairs. The keys and values may be of any valid data type. A mapping is enclosed in braces (`{}`). Its elements, the key-value pairs, are separated by a comma (`,`). The key and value are themselves separated by a colon (`:`).
...
A meta statement defines metadata for the format being described. A Dessert document must include one `meta` statement. `meta` statements are usually the first statement of a Dessert document. No more than one `meta` statement should appear in a document.

[source,go]
----
meta_stmt := "meta" mapping ";"; 
----

The "meta" statement consists in order of the keyword `meta`, a mapping, and the statement terminator `;`. The mapping inside a meta statement is referred to as the metadata mapping.
...
|dessert
|string
|(required) The minimum version that a Dessert runtime must support to be able to parse this document.
...
|endian
|string
|(required) The byte endianness of the format being described. This must either be the string "little" or the string "big", meaning little-endian byte order and big-endian or network byte order, respectively.

|root
|identifier
|(required) The root structure from which to start parsing input data. This must be an identifier referring to a structure.
|===

=== Struct statements

The `struct` statement defines a structure, an ordered sequence of named elements called fields. Each struct field has a name and a type.
...
The type name is an identifier referring to one of the following data types: 
...
[source,go]
----
type_name := unsigned_int_name | signed_int_name | float_name | byte_name |
             array_name | match_name
...
byte_name         := "byte" "[" integer "]"
array_name        := (unsigned_int_name | signed_int_name | float_name) "[" integer "]"
match_name        := ("match" "[" string "]") | string
----

Struct statements can be affected by <<Statement modifiers>>. The only available modifier is the `endian` field which if specified must be either the string `"little"` or the string `"big"`. This modifier controls the byte endianness of all integer fields contained in the struct. If the field is not specified, the endianness should be the same as the endianness of the parent structure.
