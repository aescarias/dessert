...
The `struct` statements defines a structure, an ordered sequence of named elements called fields. Each struct field has a name and a type.

[source,go]
----
struct_stmt := "struct" identifier "{" fields* "}"
fields      := field ("," field)*
