...
[source,text]
----
true      false    struct    enum    import  
meta      let      const     if      else       
switch    for      while     func    return
----
...
[source,go]
----
expr     := unary_op | binary_op | literal | function_call | grouping 
grouping := "(" expr ")"
---- 
...
- Unary tilde (`~int`) applies the bitwise complement on integer `ìnt`, that is, inverts each bit of the integer.
...
Comparison operators report whether an operand A is less than, greater than, or equal to an operand B.

Equality operators report whether two operands `A` and `B` are distinct or equal to each other, as a boolean result.

The equality operator `==` reports whether `A` and `B` are equal. The non-equality operator `!=` applies a negation to the result of `A == B`.

For two operands A and B, equality is determined as follows:

- For A and B, both integers or both floats, `A == B` reports true if the numeric value of A is equal to B. 
- For a floating-point number A and an integer B, or vice versa, `A == B` reports true if the integer form of A (the float A truncated and converted to an integer) matches the numeric value of B.
- For booleans A and B, `A == B` reports true if the boolean literals are the same.
- For strings A and B, `A == B` reports true if the length of A matches the length of B and, for each byte A~n~ and B~n~, the byte value of A~n~ matches B~n~.
- For lists A and B, `A == B` reports true if the length of A matches the length of B and, for each item A~n~ and B~n~, A~n~ is equal to B~n~.
- For mappings A and B, `A == B` reports true if the length of A matches the length of B, and for each key of A, the key exists in B and the corresponding value for the key in A matches the corresponding value of such key in B. The order of the elements is not considered in this comparison.

Except as noted above, for two operands A and B where the type of A does not match the type of B, `A == B` is false.

For strings, lists, and mappings, `A == B` is false if the length of A does not equal the length of B, regardless of the contents of A and B. Likewise, `A == B` returns false if, for any element A~n~ and B~n~, A~n~ is not equal to B~n~, regardless of the remaining contents of A and B.

The less than (`<`) operator reports whether operand A is less than operand B. The greater than (`>`) operator report whether operand A is greater than operand B. Their variants `<=` and `>=` introduce an equality component: `A <= B` is the same as `A < B || A == B` and `A >= B` is the same as `A > B || A == B`.

For two operands A and B, `<` and `>` are defined as follows:

- For A and B, both integers or floats, `A < B` is true if the numeric value of A is less than the numeric value of B. `A > B` is true if A is greater than B.
- For a floating-point number A and an integer B, or vice versa, `A < B` is true if the integer form of A (the float A truncated and converted to an integer) is less than the numeric value of B. `A > B` is true if the integer form of A is greater than the numeric value of B.

Less than and greater than operations are currently implemented for integers and floats only. Attempting to perform `<` or `>` with any other type shall raise a type error.
...
Function calls allow calling a function. Functions are procedures that perform a task based on input. Dessert currently only supports native function which are functions defined by the language runtime.

A function call consists of an expression known as the *callee*, which must resolve to a function object. The callee is followed by a set of parentheses (`()`) within which may be specified a list of expressions known as *arguments*. Arguments consist of literals that are provided, in order, as input to the function. Each argument is separated by a comma (`,`).
...
- `meta` statements.
- `struct` statements.
- `struct` field statements.
- Expression statements.

[source,go]
----
statement := meta_stmt | struct_stmt | struct_field_stmt | expr_stmt
----
...
The `struct` statement defines a structure, an ordered sequence of statements that may define named elements called fields. Fields part of struct statements are defined using <<Struct field statements>>. A struct field consists of a name, a type, and optionally, a set of modifiers.

[source,go]
----
struct_stmt := modifier_list? "struct" identifier "{" statement* "}" ";"
----
...
Struct field statements are used to define fields within a struct. It consists of an identifier and a type name separated by a colon (`:`).

[source,go]
----
struct_field_stmt := modifier_list? identifier ":" type_name
----

Struct field statements may be affected by modifiers. The modifiers available are:
